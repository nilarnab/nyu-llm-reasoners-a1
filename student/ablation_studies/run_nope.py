...
def main_training_loop(max_learning_rate,
    ...
    for it_id in tqdm(range(it_start, ITERATIONS)):
        ...
        if it_id % SAVE_CHECK_POINT_ITERATION == 0:
            save_checkpoint(model, optimizer, it_id + 1, f"{CHECKPOINT_FOLDER}/checkpoint_run_nope_SESSION{SESSION_ID}_IT{it_id}.pt")
...
if __name__ == '__main__':
    ...
    wandb.init(
        ...
        name=f"hpc-bs{BATCH_SIZE}-maxlr{max_learning_rate}-minlr{min_learning_rate}-nope",  # optional: run name
        config={
